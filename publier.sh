...
# sous licence.
#
# DIX CONTROLES AVANT DE POUSSER, et le script s'arrete au premier qui echoue.
...
echo "1/10  les portes repondent-elles ce qu'on attend ?"
python -X utf8 V3/layers/L0_interrupteur/test_portes.py

echo "2/10  et sur le vrai chemin de code, en faux live ?"
python -X utf8 V3/layers/L0_interrupteur/test_faux_live.py

echo "3/10  le calendrier bloque-t-il les bons jours ?"
python -X utf8 V3/tests/test_calendrier.py

echo "4/10  les fiches F23 decrivent-elles ce qui s'est passe ?"
python -X utf8 V3/tests/test_f23.py

echo "5/10 ce qui vient d'un proxy se refuse-t-il ?"
python -X utf8 V3/tests/test_proxys.py

echo "6/10 les ctx_* livrees se reproduisent-elles ?"
python -X utf8 V3/tests/test_ctx.py

echo "7/10 rvol_r et cvd_sess_r sont-elles dans l'agregation ?"
python -X utf8 V3/tests/test_recalc_agg.py

echo "8/10 la SPEC L3 porte-t-elle les nombres du code tague ?"
python -X utf8 V3/tests/test_spec_l3.py

echo "9/10 rien de sensible, nulle part, dans aucune version ?"
python -X utf8 V3/tests/test_structure.py

echo "10/10 le travail est-il commite ?"
