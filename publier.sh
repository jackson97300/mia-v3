...
# sous licence.
#
# ONZE CONTROLES AVANT DE POUSSER, et le script s'arrete au premier qui echoue.
...
echo "1/11  les portes repondent-elles ce qu'on attend ?"
python -X utf8 V3/layers/L0_interrupteur/test_portes.py

echo "2/11  et sur le vrai chemin de code, en faux live ?"
python -X utf8 V3/layers/L0_interrupteur/test_faux_live.py

echo "3/11  le calendrier bloque-t-il les bons jours ?"
python -X utf8 V3/tests/test_calendrier.py

echo "4/11  les fiches F23 decrivent-elles ce qui s'est passe ?"
python -X utf8 V3/tests/test_f23.py

echo "5/11 ce qui vient d'un proxy se refuse-t-il ?"
python -X utf8 V3/tests/test_proxys.py

echo "6/11 les ctx_* livrees se reproduisent-elles ?"
python -X utf8 V3/tests/test_ctx.py

echo "7/11 rvol_r et cvd_sess_r sont-elles dans l'agregation ?"
python -X utf8 V3/tests/test_recalc_agg.py

echo "8/11 les cinq vetos L4 annulent-ils sans jamais compter ?"
python -X utf8 V3/layers/L4_orderflow/test_orderflow.py

echo "9/11 la SPEC L3 porte-t-elle les nombres du code tague ?"
python -X utf8 V3/tests/test_spec_l3.py

echo "10/11 rien de sensible, nulle part, dans aucune version ?"
python -X utf8 V3/tests/test_structure.py

echo "11/11 le travail est-il commite ?"
